# 결식아동을 위한 편의점 식품 선택·영양 평가 로직 설계
...
- 회원가입·개인정보 동의·사용자 정보 입력
- 하루 및 한 끼 목표 설정
- 상품명 검색을 통한 식품 선택
- 실제 섭취량에 따른 영양소 계산
- 스택을 활용한 음식 추가·최근 입력 취소
- 한 끼·하루 영양소 합산 및 목표 비교
- 보완 식품 추천
- 식사 기록 저장 및 하루 종합 평가
- 입력·조회·계산·저장 과정의 예외 처리
...
- 성별·키·몸무게·나이 등의 정보만으로 모든 목표를 계산할 수 있다고 가정하지 않는다.
...
- 개인정보 수집 범위와 필요한 동의·보호 절차는 별도 검토한다.
...
1. 사용자가 검색어를 입력한다.
2. 검색어와 검색 대상 상품명의 공백·영문 대소문자를 일관된 방식으로 정리한다.
...
임의의 과거 기록 수정·삭제는 해당 음식 기록을 찾아 처리하고, 합계를 재계산한다. 스택 자체를 영구 저장소로 사용하지 않는다.
...
- 목표 범위가 있는 경우: 범위 미만·범위 내·범위 초과 구분
...
- 누락된 값을 0으로 가정하여 미달·적정을 확정하지 않는다.
...
추천 점수 = 보완 효과의 가중합 - 목표 범위·상한 초과에 대한 감점

- 서로 다른 영양소의 g·mg 값을 그대로 더하지 않는다.
- 영양소별 기준 대비 비율로 비교한다.
...
3. 영양소별 하루 총량을 계산한다.
4. 하루 기준과 비교한다.
5. 누락·추정 상태와 기록 범위를 반영하여 결과를 표시한다.
...
- 식사 추가·수정·삭제 후 해당 날짜의 합계를 재계산한다.
...
flowchart TD
    ...
    B -- 예 --> D[합계 및 누락·추정 상태 확인]
    D --> E[한 끼 기준과 비교]
    E --> F{평가 가능한 보완 항목이 있는가?}
    F -- 아니오 --> J[한 끼 결과 표시]
    F -- 예 --> G[추천 후보 점수 계산 및 정렬]
    G --> H{적합한 후보가 있는가?}
    H -- 예 --> I[상품·섭취량·추천 이유 표시]
    H -- 아니오 --> K[추천 없음 표시]
    I --> J
    K --> J
...
| 추정 성분 사용 | 계산·평가·추천에 추정 표시 |
| 섭취량 0·음수·숫자 아님 | 입력 수정 전 추가 차단 |
| 기준량 0·환산 근거 없음 | 해당 성분 계산 중단 |
| 빈 스택에서 취소 | 기록 변경 없이 유지 |
| 빈 식사 평가 | 음식 입력 안내 |
| 같은 상품을 다시 섭취 | 별도 섭취 기록 허용 |
| 연속 클릭·요청 재전송 | 같은 요청의 중복 반영 방지 |
| 조회·저장 실패 | 입력 유지 및 재시도 제공 |
| 앱 종료 | 저장된 임시 기록 복원 |
| 저장된 식사 수정·삭제 | 식별자로 변경 후 합계 재계산 |
| 날짜 변경 | 실제 섭취 날짜로 분리 |
| 하루 평가 후 식사 추가 | 해당 날짜 평가 갱신 |
| 추천 후보 없음 | 추천 없음과 이유 표시 |
| 일부 식사 미기록 | 기록 범위 명시 |
| 목표·성분 기준 변경 | 기존 기록의 적용 기준 보존 및 재계산 여부 명시 |
...
| 음식 입력·최근 취소 | 스택 | 마지막 기록부터 취소 |
| 한 끼·하루 합산 | 누적 및 재계산 | 영양소별 총량 산출 |
| 목표 비교 | 조건 분기 | 기준 대비 상태 구분 |
| 식품 추천 | 후보별 점수 계산 및 정렬 | 보완 가능한 후보 제시 |
| 중복 저장 방지 | 기록 식별자·요청 중복 검사 | 중복 반영 방지 |
